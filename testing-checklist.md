...
6. Login yang **masih bisa berhasil** walau email belum diverifikasi → bug.
7. Response login/Google login yang `display_name`-nya `null`/hilang padahal user sudah pernah isi nama saat register → bug.
8. `GET /surah?juz=...` yang salah menghitung `ayah_range_in_juz`, atau melewatkan surat yang lintas juz (bandingkan manual sama data mushaf) → bug.